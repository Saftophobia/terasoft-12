    ...
    class Environment3 : Microsoft.Xna.Framework.DrawableGameComponent
    {
        ...
        public void GenerateSolvable()
        {
            ...
            do
            {
                ...
                switch (x)
                {
                    case Constants3.HOLE_OUT_OF_NEAR_RANGE: if (hole.Position.X == Constants3.MAX_HOLEPOSX && hole.Position.Z != Constants3.MAX_HOLEPOSZ)
                            hole.Position = (Vector3.Add(hole.Position, new Vector3(0, 0, -1)));
                        else if (hole.Position.Z == Constants3.MAX_HOLEPOSX && hole.Position.X != Constants3.MAX_HOLEPOSZ)
                            hole.Position = (Vector3.Add(hole.Position, new Vector3(1, 0, 0)));
                        else friction++; break;
                    case Constants3.HOLE_OUT_OF_FAR_RANGE: if (hole.Position.X == Constants3.MAX_HOLEPOSX && hole.Position.Z != Constants3.MAX_HOLEPOSZ)
                            hole.Position = (Vector3.Subtract(hole.Position, new Vector3(0, 0, -1)));
                        else if (hole.Position.Y == Constants3.MAX_HOLEPOSX && hole.Position.X != Constants3.MAX_HOLEPOSZ)
