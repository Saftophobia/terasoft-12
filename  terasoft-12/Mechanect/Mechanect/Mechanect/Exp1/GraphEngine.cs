...
namespace Mechanect.Exp1
{
    class GraphEngine
    {
        ...
        public static void DrawGraphs(PerformanceGraph g, List<float> vel1, List<double> timings1, List<float> vel2, List<double> timings2, List<String> Commands, List<double> time, double player1disqtime, double player2disqtime, int gwidth, int gheight, int length, double totalTime)
        {
            g.setTotalTime(totalTime);
        ...
        public static void SetDestinations(PerformanceGraph g, int Width, int Height)
        {
            ...
            for (int j = 0; j <= 8; j++)
            {
                ...
                switch (j)
                {
                    case 0: timing = g.GetTimings1(); temporary = g.getP1Disp(); counter1 = GraphUI.Percentage(Width, 4.88); value = g.getTrackLength(); current = g.getDisplacement1(); color = Color.Red; disqList = g.getP1DispGraph(); break;
                    case 1: timing = g.GetTimings2(); temporary = g.getP2Disp(); counter1 = GraphUI.Percentage(Width, 4.88); value = g.getTrackLength(); current = g.getDisplacement2(); color = Color.Blue; disqList = g.getP2DispGraph(); break;
                    case 2: timing = g.GetTimings3(); temporary = g.getOptD(); counter1 = GraphUI.Percentage(Width, 4.88); value = g.getTrackLength(); current = g.getOptimumDisplacement(); color = Color.Yellow; break;
                    case 3: timing = g.GetTimings1(); temporary = g.getP1Vel(); counter1 = GraphUI.Percentage(Width, 37.1); value = g.getMaxVelocity(); current = g.getVelocity1(); color = Color.Red; disqList = g.getP1VelGraph(); break;
                    case 4: timing = g.GetTimings2(); temporary = g.getP2Vel(); counter1 = GraphUI.Percentage(Width, 37.1); value = g.getMaxVelocity(); current = g.getVelocity2(); color = Color.Blue; disqList = g.getP2VelGraph(); break;
                    case 5: timing = g.GetTimings3(); temporary = g.getOptV(); counter1 = GraphUI.Percentage(Width, 37.1); value = g.getMaxVelocity(); current = g.getOptimumVelocity(); color = Color.Yellow; break;
                    case 6: timing = g.GetTimings1(); temporary = g.getP1Acc(); counter1 = GraphUI.Percentage(Width, 69.33); value = g.getMaxAcceleration(); current = g.getAcceleration1(); color = Color.Red; disqList = g.getP1AccGraph(); break;
                    case 7: timing = g.GetTimings2(); temporary = g.getP2Acc(); counter1 = GraphUI.Percentage(Width, 69.33); value = g.getMaxAcceleration(); current = g.getAcceleration2(); color = Color.Blue; disqList = g.getP2AccGraph(); break;
                ...
                for (int i = 0; i <= temporary.Count - 1; i++)
                {
                    ...
                    if (j <= 2)
                    {
                        r2 = (double)(g.getTrackLength() - temporary[i]) / (double)r;
                        try
                        {
                            r4 = (double)(g.getTrackLength() - temporary[i + 1]) / (double)r;
                        }
                        catch (Exception e)
                        {
                            r4 = r2;
                    ...
                    if (j > 2 && j <= 5)
                    {
                        r2 = (double)(g.getMaxVelocity() - temporary[i]) / (double)r;
                        try
                        {
                            r4 = (double)(g.getMaxVelocity() - temporary[i + 1]) / (double)r;
                        }
                        catch (Exception e)
                        {
                            r4 = r2;
                    ...
                    if (j > 5 && j <= 8)
                    {
                        r2 = (double)(g.getMaxAcceleration() - temporary[i]) / (double)r;
                        try
                        {
                            r4 = (double)(g.getMaxAcceleration() - temporary[i + 1]) / (double)r;
                        }
                        catch (Exception e)
                        {
                            r4 = r2;
